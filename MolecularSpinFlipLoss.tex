...
The knowledge of spin flips or Majorana hops as an eventual trap lifetime limit predates the very first magnetic trapping of neutrals\s\cite{Migdall1985}. 
...
These states are shown with no electric field (a), with $|\vec{E}|\s {=}\s 150\text{ V/cm}$ and $\vec{E}\s $\raisebox{1px}{${\parallel}$}$\s\vec{B}$ (b), and with \epb{} (c). 
...
Energy splitting contours are shown every $40\text{ MHz}$ near the zero of a $2\text{ T/cm}$ magnetic quadrupole trap for OH molecules\s\cite{Stuhl2012uwave} with $\vec{E}\s {=}\s 0$ (e), and with uniform $E\s {=}\s 150\text{ V/cm}$ along the strong axis of the quadrupole (f). 
The vectors are $\mu_\text{eff}\vec{B}\s {\pm}\s  d_\text{eff}\vec{E}$, where the sign is positive above the horizontal centerline and negative below, which  represents the proper quantization axis for magnetically trapped molecules. 
Note the drastic widening of the lowest contour (red), the culprit for molecular spin-flip loss enhancement.
}
\label{fig:blocking}
\end{figure}

These internal spin-dynamics are subtle, having eluded two previous investigations of note: In\s\cite{Lara2008} the analogues of atomic spin-flip loss for molecules in mixed fields were modeled, and a magnetic quadrupole trap for OH molecules with superposed electric field was specifically addressed. 
...
In\s\cite{Bohn2013} it was correctly noted that Hund's case (a) molecules maintain a quantization axis in mixed fields. 
...
During its orbit, $\vec{B}$ continuously rotates while $\vec{E}$ maintains its lab frame orientation, so that eventually $\phi\s {<}\s 0$ and $\vec{E}$ and $\vec{B}$ are closer to anti-parallel. 
Now the length of the quantization axis $d_\text{eff}\vec{E}\s {+}\s\mu_\text{eff}\vec{B}$, which is proportional to the field induced energy shift of the molecule, actually decreases with increasing $|B|$.
The molecule is then magnetically strong field seeking and is lost.
Therefore, to remain well-trapped a molecule must have the quantization axis $d_\text{eff}\vec{E}\s {+}\s\text{sign}(\phi)\mu_\text{eff}\vec{B}$, so that an increase in magnitude of the magnetic field increases its potential energy. 
...
Since $\phi$ is a continuous scalar defined in 3D, $\phi\s {=}\s 0$ is a contour level of $\phi$ and thus always corresponds to a 2D surface defined by \epb{}; in the case of a magnetic quadrupole with homogeneous E it is a plane.
...
This reduction in the Zeeman splitting from linear to cubic is in fact a well known phenomenon in the precision measurement community\s\cite{Player1970,Hudson2002}, and experimentalists have exploited it for the suppression of unwanted influence from magnetic fields in measurements of the electron electric dipole moment.
...
\begin{table}[t]
\caption{
Enhancements ($\eta$) and loss rates ($\s amma$) for OH with typical applied fields. 
Zero field values are equivalent to atomic spin-flip loss. 
Electric field is required during evaporation and spectroscopy to open avoided crossings for $|e\rangle$ parity states\s\cite{Stuhl2012evap,Stuhl2012uwave}, or applied for polarization of the molecules to study collisions\s\cite{Stuhl2013}. Background loss is $2\s\text{s}^{-1}$, experiment length $100\s\text{ms}$.
}
\label{tab:rates}
...
\end{equation}
which relates the probability of diabatically hopping between two states $P_\text{hop}$ to the energetic coupling between the states $\delta$ and the rate of approach of the energy levels $\dot{G}\s {=}\s v_zdG/dz$. 
Here $z$ and $v_z$ are normal to the plane, and we neglect the component of $\dot{G}$ due to the other coordinates since from Eqn.\s\ref{eqn:energetics} it is clear that $dG/dz$ dominates.
...
We perform these integrations for OH in a $2\text{ T/cm}$ magnetic quadrupole\s\cite{Sawyer2008} under various electric fields of interest (Tab.\s\ref{tab:rates}).

It is also possible to proceed algebraically, which is useful for understanding how the effect scales.
...
\end{equation}
See Sec.\s 3 of\s\cite{ssm} for the full derivation. 
...
Returning to the numerical approach, we note that the direct integration of flux is a key improvement relative to our previous work\s\cite{Stuhl2013}, where electric fields were applied in our magnetic quadrupole trap to study their effect on collisions.
At that time the mechanism of molecular spin-flip loss was correctly identified in\s\cite{Stuhl2013}, and an attempt was made to deconvolve it from the collisional effect. 
...
Revisiting this with the direct integration of flux, we find a three-fold larger loss magnitude, enough to explain a significant portion of the effect previously attributed to collisions, see Sec.\s 1 of\s\cite{ssm}.
...
We now present a new trap where the enhanced molecular spin-flip loss can be conclusively demonstrated without any convolved collisional effects, and where our loss-flux calculations can be clearly tested.
...
\caption{
The last six pins of our Stark decelerator\s\cite{Sawyer2008} form the trap (a), which is $0.45\text{ K}$ deep with trap frequency $\nu\s {\approx}\s 4\text{ kHz}$ (b). 
...
This is shown for the $x\s {=}\s 0$ plane (c), with yellow pins artificially projected for clarity since they don't actually intersect the plane. 
The central pins are magnetized, with two domains each. 
Blue indicates magnetization along $+\hat{y}$, red along $-\hat{y}$. 
These domains produce a magnetic quadrupole trap with zero along the $z$-axis, shown in the $z\s {=}\s 0$ plane (d). 
\label{fig:CAD}}
\end{figure}
...
In Fig.\s\ref{fig:LSurfs}, the surfaces where \epb{} for several \bcl{} magnitudes are calculated and shown wherever $G<=\kappa$.
...
This translation serves to disrupt the idealized 2D magnetic quadrupole by adding a small trapping field $\vec{B} \s {\propto}\s  B^\prime z\hat{z}$. 
...
The fits are performed using only temperature as a free parameter, which enters the calculation via the thermal distribution used for integration, and fits to $170\s {\pm}\s 20\s\text{mK}$
\s\footnote{Calculation performed in COMSOL: \href{https://github.com/dreens/spin-flip-integration/}{Source Code}}.
