...
\caption{
Depletion spectroscopy spectra are obtained after evaporation (blue filled circles) and without evaporation (red open circles). The integrated areas under the curves correspond to the total number of molecules detected. Solid lines are fits to Maxwell-Boltzmann distributions with temperatures of $59\pm2$~mK without evaporation (red), and $30\pm2$~mK for evaporation (blue). The evaporation achieves a clear density enhancement in the vicinity of $500\text{ G}$.
}
...
We consistently observe this at the $(6\,{\pm}\,2)\%$ level, suggesting an evaporative effect despite the negative influence of spin-flip losses.%, note the height difference in the traces shown in Fig.~\ref{fig:normenhance}.
...
\begin{split}
\label{eqn:eta}
\eta\quad =&\quad \left(\frac{1}{\mu_\text{eff}B'}\sqrt[3]{\frac{8\kappa(d_\text{eff}E)^4}{\beta\Delta^2}}\right)^2\bigg/\alpha\left(\frac{2\kappa}{\mu_\text{eff}B'}\right)^2\\
